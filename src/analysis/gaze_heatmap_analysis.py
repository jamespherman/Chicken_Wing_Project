...
def analyze_single_subject(csv_path, output_dir=None, subject_name=None, config=None):
    """
    Convenience function to analyze a single subject.
        
    Args:
        csv_path (str): Path to the final gaze CSV file
        output_dir (str): Output directory (defaults to same as CSV)
        subject_name (str): Subject name (auto-detected if None)
    ...
    analyzer = GazeHeatmapAnalyzer(config)
    return analyzer.analyze_subject(csv_path, output_dir, subject_name)
...
def main():
    ...
    csv_path = "./batch_processing_results/20231027T174922Z_ChickenWing/20231027T174922Z_ChickenWing_final_gaze_data.csv"
    output_dir = "./batch_processing_results/20231027T174922Z_ChickenWing/"
        
    try:
        results = analyze_single_subject(csv_path, output_dir, config=config)
            
        if results['success']:
            print(f"\nSUCCESS: Created {len(results['visualizations_created'])} visualizations!")
            print(f"Valid gaze points: {results['statistics']['filtered_samples']:,}")
            print(f"Data quality: {results['statistics']['filtered_percentage']:.1f}%")
            return True
        else:
